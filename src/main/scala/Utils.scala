...
object Utils extends Serializable {
  ...
  def readCsv(address: String, sqlContext: SparkSession): DataFrame = {
    ...
    println("Follows are the train data：")
  ...
  def readJson(address: String, sqlContext: SQLContext): DataFrame = {
    ...
    //df.show(2)//print the first 5 rows
    //println("-"*70)
    //df.printSchema()
    //println("-"*70)
    return df
  ...
  def readHive(table: String, hiveContext: SparkSession): DataFrame = {

    val df = hiveContext.sql("SELECT * FROM tmp_payquerydb.xlrao_FactBase_Cust_bigtable_score1 WHERE uid_uid IS NOT NULL")
    return df
  }
  ...
  def rfrModelling(data: DataFrame, flag: String): PipelineModel = {
    ...
    //prediction.select("prediction","Label").show(2)
  ...
  /**
    ...
    * @return pipeline
    */
  def rfcModelling(data: DataFrame, flag: String): CrossValidatorModel = {
    /** *
      * Algorithm Flow:
      * read data->
      * create pipeline (chain the data pre processing function)->
...
//      .setMaxDepth(20) //set the tree depth
    ...
    // test the pipeline Parameter tunning
    val paramGrid = new ParamGridBuilder()
      .addGrid(rf_model.numTrees,Array(30))
      .addGrid(rf_model.maxDepth,Array(30))
      .build()
    ...
    println(modelRFC.getEstimatorParamMaps.toList)
    //print the first 5 predict result if you need
    //prediction.show(10)
  ...
  /**
    ...
    */
  def gbtcModelling(data: DataFrame, flag: String): CrossValidatorModel = {
    /** *
      * Algorithm Flow:
      * read data->
      * create pipeline (chain the data pre processing function)->
...
//      .setImpurity("entropy") //add noise to improve the robust
    ...
    val paramGrid = new ParamGridBuilder()
      .addGrid(gbt_model.maxIter,Array(30))
      .addGrid(gbt_model.maxDepth,Array(20))
      .build()
    ...
    // train the model
    // the process included the indexing process and fit model process
    val model_GBTC = cv.fit(trn_data)
  ...
  def predictAndProbability(prediction: DataFrame): DataFrame = {
    ...
    val proba = probability.map { case Row(row0: String, row1: Vector) => (row0, row1(0), row1(1)) }
  ...
  def outliersBoxProcess(dataFrame: DataFrame,flagList:List[String]): DataFrame = {
    var newdata = dataFrame
    for (flag <- flagList:List[String]) {
  ...
  def normalDataProcess(dataFrame: DataFrame,flagList:List[String]): DataFrame = {
    var newData = dataFrame
    for (flag <- flagList) {
  ...
  def outliersProcess(dataFrame: DataFrame, flagList:List[String],replace: Int = 95): DataFrame = {
    var newData = dataFrame
    for (flag <- flagList:List[String]) {
      val quantiles = dataFrame.select(flag)
